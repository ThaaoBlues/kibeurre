...
impl<const VECTOR_SIZE : usize> Vector<{VECTOR_SIZE}> {
    ...
    pub fn dot(&mut self, v2 : Vector<VECTOR_SIZE>) -> i32{
        ...
        for i in 0..VECTOR_SIZE {
            ...
            ret = (((ret + (self.c[i] * v2.c[i]) % self.m) % self.m) + self.m) % self.m;
        }
    ...
    pub fn scalar_mult(&mut self,v : i32){
        ...
        for i in 0..VECTOR_SIZE {
            ...
            self.c[i] = ((self.c[i] * v) % self.m + self.m) % self.m;
        } 
    ...
    pub fn encode(&self, nb_bits : usize)->Vec<u8>{
        // encode coefficients on 12 bits 

        let bit_size = nb_bits*POLYNOMIAL_SIZE;
        let byte_size = bit_size.div_ceil(8);

        let mut ret : Vec<u8> = vec![0u8;byte_size];
        let mut bit_index = 0;
...
impl<const VECTOR_SIZE : usize> PolyVector<{VECTOR_SIZE}> {
    ...
    pub fn ntt_dot(&self, v2 : PolyVector<VECTOR_SIZE>) -> Vector<POLYNOMIAL_SIZE>{
        ...
        */
        ...
        // get each polynomial of the first vector
   ...
   pub fn encode(&self,nb_bits : usize)->Vec<u8>{
    // encode each polynomial coefficients on 12 bits
        let mut ret : Vec<u8> = Vec::new();
        for i in 0..k{
            ret.extend(self.c[i].encode(nb_bits));
        }
...
impl <const RN : usize, const CN : usize> PolyMatrix<{RN},{CN}>{
    ...
    pub fn set_coef(&mut self, row_index : usize, col_index : usize, v : Vector<POLYNOMIAL_SIZE>){
...
#[test]
fn test_ntt_dot_with_known_scalars() {

    // Vector 1: [ PolynomialA, PolynomialB ]
    // PolyA = 3, PolyB = 4
    let mut poly_a = [0; POLYNOMIAL_SIZE];
    poly_a[0] = 3;
    let mut poly_b = [0; POLYNOMIAL_SIZE];
    poly_b[0] = 4;
    
    // Vector 2: [ PolynomialC, PolynomialD ]
    // PolyC = 5, PolyD = 6
    let mut poly_c = [0; POLYNOMIAL_SIZE];
    poly_c[0] = 5;
    let mut poly_d = [0; POLYNOMIAL_SIZE];
    poly_d[0] = 6;
    ...
    let result = v1.ntt_dot(v2);

    assert_eq!(result.c[0], 39, "The scalar dot product component at index 0 should be 39.");

    // Check that all other coefficients remain 0
    for i in 1..POLYNOMIAL_SIZE {
        assert_eq!(
            result.c[i], 0, 
            "Coefficient at index {} should be 0 for scalar inputs.", i
        );
    }
}

#[test]
fn test_ntt_dot_modulo_wrap() {
    // Test that the function correctly wraps under modulo 3329
    let mut poly_a = [0; POLYNOMIAL_SIZE];
    poly_a[0] = 2000;
    let mut poly_c = [0; POLYNOMIAL_SIZE];
    poly_c[0] = 2000;
    ...
    // but our addition inside ntt_dot must also handle it.
    // Assuming your ntt::poly_mult(2000, 2000) correctly outputs (2000 * 2000) % 3329 = 682
    // Let's use simpler numbers to test *just* your addition logic wrapping:
    
    let mut p_a = [0; POLYNOMIAL_SIZE]; p_a[0] = 3000;
    let mut p_b = [0; POLYNOMIAL_SIZE]; p_b[0] = 1000;
    // If poly_mult returns them effectively as identities:
    // 3000 + 1000 = 4000 -> 4000 % 3329 = 671
    
    // (For a bulletproof test of the accumulator, mock or track the exact output of your poly_mult)
}
}
